...
function computeVertexNormals( coordsArray, normalsArray ) {
    ...
    for( var index = 0; index < coordsArray.length; index += 9 )
    {
        ...
        for( var j = 0; j < 3; j++ )
        {
			if (normalVector[0] < 0) normalVector[0] = -1;
			else if (normalVector[0] > 0) normalVector[0] = 1;
			else normalVector[0] = 0;
			if (normalVector[1] < 0) normalVector[1] = -1;
			else if (normalVector[1] > 0) normalVector[1] = 1;
			else normalVector[1] = 0;
			if (normalVector[2] < 0) normalVector[2] = -1;
			else if (normalVector[2] > 0) normalVector[2] = 1;
			else normalVector[2] = 0;
